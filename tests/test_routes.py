...
class TestProductRoutes(TestCase):
    ...
    def test_get_product(self):
        ...
        # make a self.client.get request to the API endpoint and store the result in
        # the variable named response
    ...
    def test_update_product(self):
        ...
        test_product = ProductFactory()
        # send a self.client.post() request to the BASE_URL with a json payload
        # of test_product.serialize()
        response = self.client.post(BASE_URL, json=test_product.serialize())
    ...
    def test_delete_product(self):
        ...
        # call the self.get_product_count() method to retrieve the initial count
        # of products before any deletion
        ...
        # send a self.client.get request to the same endpoint that was deleted to
        # retrieve the deteled product
        response = self.client.get(f"{BASE_URL}/{test_product.id}")
        # assert that the resp.status_code is status.HTTP_404_NOT_FOUND to confirm
    ...
    def test_query_by_name(self):
        ...
        products = self._create_products(5)
        # extract the name of the first product in the products list and assigns
        # it to the variable test_name
    ...
    def test_query_by_category(self):
        ...
        # assert that response status code is 200, indicating a successful request
        # (HTTP 200 OK)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        # retrieve the JSON data from the response
        data = response.get_json()
        # assert that the length of the data list (i.e., the number of products
        # returned in the response) is equal to found_count
    ...
    def test_query_by_availability(self):
        ...
        data = response.get_json()
        # assert that the length of the data list (i.e., the number of products
        # returned in the response) is equal to available_count
        self.assertEqual(len(data), available_count)
        # check the data just to be sure
        # use a for loop to check each product in the data list and verify that
        # the "available" attribute of each product is set to True
        for product in data:
