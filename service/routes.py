...
def list_products():
    ...
    # to the corresponding enum value from the Category enumeration
        ...
        category_value = getattr(Category, category.upper())
         # call the Product.find_by_category(category_value) method to retrieve 
         # products that match the specified category_value
        products = Product.find_by_category(category_value)
    # If you did, convert the available string retrieved from the query parameters 
    # to a boolean value
    elif available:
        app.logger.info("Find by available: %s", available)
        # create bool from string
        available_value = available.lower() in ["true", "yes", "1"]
        # call the Product.find_by_availability(available_value) method to 
        # retrieve products that match the specified available_value
